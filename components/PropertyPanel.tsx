...
import { Bold, Plus, Minus, Type } from 'lucide-react';
...
export const PropertyPanel: React.FC<PropertyPanelProps> = ({ data, selection, onUpdateStyle }) => {
  ...
  const currentSize = selectedCell?.style?.fontSize || 3.1;
  const isLinkedCol = selection.colIdx >= 1 && selection.colIdx <= 7;
      ...
      <div className="space-y-6">
        <div className="flex flex-col gap-3">
          <label className="text-[9px] font-bold uppercase text-gray-400 flex items-center gap-2">
            <Type size={12} className="text-gray-600" /> Размер шрифта (мм)
          </label>
          <div className="flex items-center justify-between bg-black/40 rounded-lg p-1 border border-white/5">
            <button 
              onClick={() => onUpdateStyle({ fontSize: Math.max(0.5, currentSize - 0.1) })}
              className="p-2 hover:bg-white/5 rounded-md text-gray-400 hover:text-white transition-colors"
            >
              <Minus size={14} />
            </button>
            <span className="text-[11px] font-bold text-white tabular-nums">{currentSize.toFixed(1)}</span>
            <button 
              onClick={() => onUpdateStyle({ fontSize: Math.min(20, currentSize + 0.1) })}
              className="p-2 hover:bg-white/5 rounded-md text-gray-400 hover:text-white transition-colors"
            >
              <Plus size={14} />
            </button>
            ...
            {[400, 700].map(weight => (
              <button
                ...
                className={`py-2 rounded border text-[10px] font-bold uppercase transition-all ${
                  (selectedCell?.style?.fontWeight || 400) == weight
